...
export class DownloadsFlyout {
  ...
  private createWindow(): void {
    ...
    if (is.dev && process.env['ELECTRON_RENDERER_URL']) {
      this.win.loadURL(`${process.env['ELECTRON_RENDERER_URL']}#downloads-flyout`)
    } else {
  ...
  open(anchorBounds?: AnchorBounds): void {
    ...
    this.isOpen = true
    this.win.webContents.send(IPC.downloadsFlyoutShow)
    this.win.show()
    this.win.focus()

    this.parent.webContents.send(IPC.downloadsFlyoutChanged, true)
  }
  ...
  close(): void {
    ...
    if (this.win && !this.win.isDestroyed()) {
      this.win.webContents.send(IPC.downloadsFlyoutClose)
    ...
    if (!this.parent.isDestroyed()) {
      this.parent.webContents.send(IPC.downloadsFlyoutChanged, false)
    }
  }
